...
function getSignalThresholds(mode: BuyScanMode): SignalThresholds {
  if (mode === "soft") {
    return {
      mode,
      stage2StaticScoreMin: 67,
      stage2BuyScoreMin: 69,
      minimumRoomAtrMultiplier: 1.25,
      minimumRoomRiskMultiplier: 1.35,
      minimumRoomFloor: 2.9,
      rrMin: 1.35,
      stopMin: 1.5,
      stopMax: 6.6,
      hardRejectDailyRsi: 72,
      hardRejectOneHourRsi: 74,
      hardRejectFourHourAdx: 13,
      hardRejectPullbackFromHigh: 1.2,
      hardRejectChange30d: 135,
      hardRejectLateEntryRsi: 72,
      setupIdealDistanceFromEma20: 3.2,
      setupLateDistanceFromEma20: 5.6,
      rangeHighPenalty: 84,
      pullbackHealthyMin: 2.5,
      pullbackHealthyMax: 22,
      pullbackPenaltyHigh: 26,
      confirmVolumeMin: 0.88,
      confirmStrongCloseThreshold: 0.5,
      confirmBodyAtrFactor: 0.14,
      confirmRetestToleranceAtr: 0.38,
      confirmRetestTolerancePrice: 0.0032,
      confirmBreakoutRetestToleranceAtr: 0.42,
      confirmBreakoutRetestTolerancePrice: 0.0034,
      confirmTriggerBuffer: 0.994,
      confirmPreviousCloseBuffer: 0.991,
      confirmWaitingRetestCloseBuffer: 0.989,
      confirmWaitingRetestLowBufferMultiplier: 1.2,
      noDataPenalty: -7,
      waitingBreakoutRetestPenalty: -3,
      waitingRetestPenalty: -3,
      waitingClosePenalty: -4,
      strongCloseReward: 8,
      retestReward: 10,
  ...
  return {
    mode,
    stage2StaticScoreMin: 70,
    stage2BuyScoreMin: 72,
    minimumRoomAtrMultiplier: 1.4,
    minimumRoomRiskMultiplier: 1.55,
    minimumRoomFloor: 3.3,
    rrMin: 1.5,
    stopMin: 1.7,
    stopMax: 5.8,
    hardRejectDailyRsi: 70,
    hardRejectOneHourRsi: 71,
    hardRejectFourHourAdx: 15,
    hardRejectPullbackFromHigh: 2,
    hardRejectChange30d: 110,
    hardRejectLateEntryRsi: 69,
    setupIdealDistanceFromEma20: 2.6,
    setupLateDistanceFromEma20: 4.8,
    rangeHighPenalty: 81,
    pullbackHealthyMin: 3.5,
    pullbackHealthyMax: 20,
    pullbackPenaltyHigh: 24,
    confirmVolumeMin: 0.92,
    confirmStrongCloseThreshold: 0.54,
    confirmBodyAtrFactor: 0.16,
    confirmRetestToleranceAtr: 0.3,
    confirmRetestTolerancePrice: 0.0025,
    confirmBreakoutRetestToleranceAtr: 0.36,
    confirmBreakoutRetestTolerancePrice: 0.0029,
    confirmTriggerBuffer: 0.996,
    confirmPreviousCloseBuffer: 0.993,
    confirmWaitingRetestCloseBuffer: 0.992,
    confirmWaitingRetestLowBufferMultiplier: 1.08,
    noDataPenalty: -9,
    waitingBreakoutRetestPenalty: -5,
    waitingRetestPenalty: -6,
    waitingClosePenalty: -7,
    strongCloseReward: 8,
    retestReward: 10,
    breakoutRetestReward: 12,
  };
...
export function evaluateMarketSignal(
  ...
  if (intraday1h.rsi14 !== null) {
    ...
    } else if (intraday1h.rsi14 > (mode === "soft" ? 70 : 68)) {
      setupScore -= mode === "soft" ? 4 : 6;
      negatives.push(
        mode === "soft"
          ? "1H RSI повышен, но soft-режим допускает более горячий импульс с умеренным penalty"
          : "1H RSI уже повышен, поэтому hard-режим штрафует сетап, но не режет слишком рано"
      );
